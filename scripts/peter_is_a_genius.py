# import json

# with open("centroids.json", "r") as f:
#     data = json.load(f)
    
# small_centr = data["centroids"][:1000]

# small_centroids = {"centroids" : small_centr}

# with open("small_centroidos.json", "w") as f:
#     json.dump(small_centroids, f, indent=4)
    
# with open("coordinates.json", "r") as f:
#     data = json.load(f)

# small_coords = data["coordinates"][:10000]

# small_coordinates = {"coordinates" : small_coords}

# with open("small_coordinatos.json", "w") as f:
#     json.dump(small_coordinates, f, indent=4)